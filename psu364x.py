...
class PSU:
    ...
    def __init__(self, address, port, baudrate, error=False):
        ...
        try:
            with open(self.GLOBAL_VARIABLES, 'r') as file:
                dx = json.load(file)
            self._voltage = float(dx["VOLT"])*1000
        except:
            self._voltage = 0
        self.model = ""
        self.SERIALNUM = ""
    ...
    def set_voltage(self,volt):
        ...
        if not os.path.exists(self.GLOBAL_VARIABLES):
            ...
            print(f"Archivo {self.GLOBAL_VARIABLES} creado")
        else:
            datos = {
                "VOLT": volt
            }
            ...
            print(f"Archivo {self.GLOBAL_VARIABLES} ya existe")
    ...
    def send(self, command, parameters=None,ONOFF = "OFF"):
